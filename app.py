...
def generate_data_file_for_year(year, league_id):
    ...
    try:
        ...
        # Fetch weekly scores for variance chart (all seasons)
        # For past seasons, get all 17 weeks. For current, get completed weeks.
        end_week_for_scores = 18 if not is_current else current_week
        raw_weekly_scores = data_fetcher.get_weekly_scores_for_season(league_id, roster_map, end_week_for_scores)
        ...
        file_path = os.path.join(BASE_DIR, f"data_{year}.json")
        with open(file_path, "w") as f:
            json.dump(year_data, f, indent=4)
        
        print(f"--- Finished generating {file_path} ---")
    except Exception as e:
        print(f"An unexpected error occurred in generate_data_file_for_year for {year}: {e}")
        traceback.print_exc()
        sys.exit(1)
...
def main():
    ...
    try:
        print("Starting data fetch for league: " + LEAGUE_ID)
        ...
        for year in league_ids.keys():
            old_file = os.path.join(BASE_DIR, f"data_{year}.json")
